...
class AbstractModel
{
    ...
    const DEFAULT_DateTimeInterface= "D, d M y H:i:s O" ;
...
//
//    public static function getAutoIcrement()
//    {
//
//
//
//        $sql = 'SELECT MainaccountId FROM  ' . static::$tableName . ' ORDER BY id DESC LIMIT 1' ;
////     $sql = 'SELECT AUTO_INCREMENT FROM ' . static::$tableName . '  WHERE ' . static::$primaryKey . ' = ' . static::$primaryKey;
////        $sql = 'SELECT AUTO_INCREMENT  FROM INFORMATION_SCHEMA.TABLES WHERE  table_name = "' . static::$tableName . '" ';
//        $stmt = DatabaseHandler::factory()->prepare($sql);
//        var_dump($stmt);
//        if ($stmt->execute() === true) {
//            if (method_exists(get_called_class(), '__construct')) {
//                $obj = $stmt->fetchAll(\PDO::FETCH_CLASS | \PDO::FETCH_PROPS_LATE, get_called_class(), array_keys(static::$tableSchema));
//                var_dump('oooooooooooooooook execeute');
//            } else {
//                $obj = $stmt->fetchAll(\PDO::FETCH_CLASS, get_called_class());
//            }
//            return !empty($obj) ? array_shift($obj) : false;
//            var_dump('nooooooooooooooooooot execeute');
//        }
//        return false;
//    }
