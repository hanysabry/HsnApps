...
protected $_format = 'A4';        // locale of the messages
    protected $_encoding = '';         // page orientation : Portrait ou Landscape
    protected $_unicode = true;        // page format : A4, A3, ...
    protected $_testTdInOnepage = true;          // charset encoding
    protected $_testIsImage = true;        // means that the input text is unicode (default = true)
protected $_fallbackImage = null;        // test of TD that can not take more than one page
    protected $_parsePos = 0;        // test if the images exist or not
    protected $_tempPos = 0;        // fallback image to use in img tags
protected $_page = 0;           // position in the parsing
    protected $_subHtml = null;           // temporary position for complex table
    protected $_subPart = false;           // current page number
protected $_subHEADER = array();        // sub html
    protected $_subFOOTER = array();       // sub Html2Pdf
    protected $_subSTATES = array();     // sub action to make the header
    protected $_isSubPart = false;     // sub action to make the footer
    protected $_isInThead = false;     // array to save some parameters
protected $_isInTfoot = false;       // flag : in a sub html2pdf
    protected $_isInOverflow = false;       // flag : in a thead
    protected $_isInFooter = false;       // flag : in a tfoot
    protected $_isInDraw = null;       // flag : in a overflow
    protected $_isAfterFloat = false;       // flag : in a footer
    protected $_isInForm = false;        // flag : in a draw (svg)
    protected $_isInLink = '';       // flag : is just after a float
    protected $_isInParagraph = false;       // flag : is in a float. false / action of the form
    protected $_isForOneLine = false;          // flag : is in a link. empty / href of the link
    protected $_maxX = 0;       // flag : is in a paragraph
    protected $_maxY = 0;       // flag : in a specific sub html2pdf to have the height of the next line
protected $_maxE = 0;           // maximum X of the current zone
    protected $_maxH = 0;           // maximum Y of the current zone
    protected $_maxSave = array();           // number of elements in the current zone
    protected $_currentH = 0;           // maximum height of the line in the current zone
    protected $_defaultLeft = 0;     // save the maximums of the current zone
        protected $_defaultTop = 0;           // height of the current line
    protected $_defaultRight = 0;           // default marges of the page
    protected $_defaultBottom = 0;
protected $_defaultFont = null;
protected $_margeLeft = 0;
        protected $_margeTop = 0;        // default font to use, is the asked font does not exist
    protected $_margeRight = 0;           // current marges of the page
    protected $_margeBottom = 0;
protected $_marges = array();
protected $_pageMarges = array();
    protected $_background = array();     // save the different marges of the current page
    protected $_hideHeader = array();     // float marges of the current page
    protected $_hideFooter = array();     // background informations
protected $_firstPage = true;     // array : list of pages which the header gonna be hidden
    protected $_defList = array();     // array : list of pages which the footer gonna be hidden
    protected $_lstAnchor = array();        // flag : first page
    protected $_lstField = array();     // table to save the stats of the tags UL and OL
protected $_lstSelect = array();     // list of the anchors
    protected $_previousCall = null;     // list of the fields
    protected $_sentenceMaxLines = 1000;     // list of the options of the current select
        /**
     * list of tag definitions
     * @var ExtensionInterface[]
     */
    protected $extensions = array();        // last action called
    /**
     * List of tag objects
     * @var TagInterface[]
     */
    protected $tagObjects = array();        // max number of lines for a sentence
    /**
     * @var bool
     */
    protected $extensionsLoaded = false;        // object html2pdf prepared in order to accelerate the creation of sub html2pdf
        /**
     * @var Debug
     */
    private $debug;     // static table to prepare the nested html tables
    /**
     * @var HtmlLexer
     */
    private $lexer;
